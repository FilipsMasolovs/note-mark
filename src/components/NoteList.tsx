import React from 'react'

import { Note } from '../app/page'

interface NoteListProps {
	notes: Note[]
	selectedNoteId: string | null
	onAddNote: () => void
	onSelectNote: (id: string) => void
	onDeleteNote: (id: string) => void
}

const NoteList: React.FC<NoteListProps> = ({ notes, selectedNoteId, onAddNote, onSelectNote, onDeleteNote }) => {
	return (
		<div>
			<button onClick={onAddNote} className='createNewNote'>
				<svg width='48' height='48' viewBox='0 0 24 24' fill='none'>
					<path d='M15 12L12 12M12 12L9 12M12 12L12 9M12 12L12 15' stroke='#0070f3' strokeWidth='1.5' strokeLinecap='round' />
					<path
						d='M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7'
						stroke='#0070f3'
						strokeWidth='1.5'
						strokeLinecap='round'
					/>
				</svg>
			</button>
			<ul className='notes'>
				{notes.map((note) => (
					<li key={note.id} className={`note ${note.id === selectedNoteId ? 'selected' : ''}`} onClick={() => onSelectNote(note.id)}>
						<span>{note.title || 'Untitled'}</span>
						<button
							onClick={(e) => {
								e.stopPropagation()
								onDeleteNote(note.id)
							}}
							className='noteDeleteButton'
						>
							<svg width='32' height='32' viewBox='0 0 24 24' fill='none'>
								<path d='M20.5001 6H3.5' stroke='#0070f3' strokeWidth='1.5' strokeLinecap='round' />
								<path
									d='M6.5 6C6.55588 6 6.58382 6 6.60915 5.99936C7.43259 5.97849 8.15902 5.45491 8.43922 4.68032C8.44784 4.65649 8.45667 4.62999 8.47434 4.57697L8.57143 4.28571C8.65431 4.03708 8.69575 3.91276 8.75071 3.8072C8.97001 3.38607 9.37574 3.09364 9.84461 3.01877C9.96213 3 10.0932 3 10.3553 3H13.6447C13.9068 3 14.0379 3 14.1554 3.01877C14.6243 3.09364 15.03 3.38607 15.2493 3.8072C15.3043 3.91276 15.3457 4.03708 15.4286 4.28571L15.5257 4.57697C15.5433 4.62992 15.5522 4.65651 15.5608 4.68032C15.841 5.45491 16.5674 5.97849 17.3909 5.99936C17.4162 6 17.4441 6 17.5 6'
									stroke='#0070f3'
									strokeWidth='1.5'
								/>
								<path
									d='M18.3735 15.3991C18.1965 18.054 18.108 19.3815 17.243 20.1907C16.378 21 15.0476 21 12.3868 21H11.6134C8.9526 21 7.6222 21 6.75719 20.1907C5.89218 19.3815 5.80368 18.054 5.62669 15.3991L5.16675 8.5M18.8334 8.5L18.6334 11.5'
									stroke='#0070f3'
									strokeWidth='1.5'
									strokeLinecap='round'
								/>
							</svg>
						</button>
					</li>
				))}
			</ul>
		</div>
	)
}

export default NoteList
